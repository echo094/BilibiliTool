...
- 十周年内容
  ...
  * 33 批量组CP
    ...
    对导入的账号按照顺序组CP（以及组的除外）
  
    对于已有CP的会刷新见证者列表
  
  * 34 批量领取点赞任务
  
    会操作所有导入的账号，任务编号导出到文件
  ...
  * 36 批量添加见证者
    ...
    给指定的队伍添加见证者，添加的账号从列表里从前往后选取
  
  * 37 自动化完成1轮任务
  
    **第一次运行前必须运行一次33获取当前队伍信息 关闭软件前导出数据**
  
    首先会将前2个账号标记为根账号，生成下属图谱
